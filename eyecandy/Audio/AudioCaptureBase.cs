...
public abstract class AudioCaptureBase : IDisposable
{
    ...
    /// <summary>
    /// Indicates whether the NewAudioDataCallback should reference Buffers exposed
    /// by SyntheticDataGenerator instead of the active audio capture processor.
    /// </summary>
    public bool UsingSyntheticData = false;

    /// <summary>
    /// When UsingSyntheticData is true, a period of silence has been detected and
    /// the callback should read SyntheticDataGenerator.Buffers instead.
    /// </summary>
    public AudioCaptureSyntheticData SyntheticDataGenerator;
    ...
    /// </summary>
    public AudioProcessingRequirements Requirements;
    ...
    // don't evaluate volume for silence until one complete buffer has been accumulated
    private bool RmsBufferFilled = false;

    private bool IsSilent = false;
    private DateTime SilenceStarted = DateTime.MaxValue; // internal, not in Buffers/InternalBuffers until minimum time passes
    private CancellationTokenSource ctsSynthetic = null; // generated by CancellationTokenFactory
    ...
    /// for performance and/or thread-safety considerations and would not be updated. Derived
    /// classes must use the protected access level and can only be created with the static
    /// Factory method provided by this base class.
    /// </summary>
    protected AudioCaptureBase(EyeCandyCaptureConfig configuration)
    {
        ...
        Requirements = new()
        {
            CalculateVolumeRMS = Configuration.DetectSilence,
        };
    ...
    /// Task.Run to ensure shutdown is completed. Derived classes should invoke this base method
    /// before doing anything else. Exiting Capture should not require Disposal and should not
    /// prohibit additional invocations of Capture on the same instance of the class.
    /// </summary>
    ...
    /// <summary>
    /// Derived classes should call this in their Capture loop, ideally just after InternalBuffers
    /// are swapped back into the publicly-available Buffers (which this method references). The
    /// method has a built-in time delay, so it will exit quickly if it is called too frequently.
    /// </summary>
    protected void DetectSilence()
    {
        ...
        if (RmsBufferFilled && Configuration.DetectSilence)
        {
            if (IsSilent)
            {
                // End silence?
                if (Buffers.RealtimeRMSVolume > Configuration.MaximumSilenceRMS)
                {
                    IsSilent = false;
            ...
            else
            {
                if (Buffers.RealtimeRMSVolume <= Configuration.MaximumSilenceRMS)
                {
                    ...
                    {
                        // Signal silent period
                        if (DateTime.Now.Subtract(SilenceStarted).TotalSeconds >= Configuration.MinimumSilenceSeconds)
                        {
                            IsSilent = true;
                ...
                {
                    // Expire wait period for silence
                    if (SilenceStarted != DateTime.MaxValue)
                    {
                        SilenceStarted = DateTime.MaxValue;
        ...
        if (IsSilent)
        {
            if (SyntheticDataGenerator is not null
                && !UsingSyntheticData
